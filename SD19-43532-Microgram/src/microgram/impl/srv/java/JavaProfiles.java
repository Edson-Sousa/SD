...
public class JavaProfiles extends RestResource implements microgram.api.java.Profiles {
	...
	public Result<Void> deleteProfile(String userId) {
		...
		for( String user: userFollowers ) {
			...
			aux.remove(userId);
			following.put(user, aux);
		}
		Set<String> userFollowing = following.remove(userId);
		for( String user: userFollowing ) {
			Set<String> aux = followers.get(user);
			aux.remove(userId);
			followers.put(user, aux);
		}
		return ok();
	}
